...
function generateFallbackResponse(userMessage) {
    ...
    if (lowerMessage.includes('比較') || lowerMessage.includes('vs') || lowerMessage.includes('対')) {
        return `選手比較の分析を行います。\n\n${userMessage}について、以下の観点から分析いたします：\n\n1. **ポジションと役割**\n2. **技術的な特徴**\n3. **統計データの比較**\n4. **チームへの貢献度**\n5. **今後の可能性**\n\nより詳細な分析をご希望の場合は、Gemini APIキーを設定してください。`;
    }
    
    // 試合予測
    if (lowerMessage.includes('試合') || lowerMessage.includes('予測') || lowerMessage.includes('結果')) {
        return `試合予測の分析を行います。\n\n${userMessage}について、以下の観点から分析いたします：\n\n1. **両チームの現在の調子**\n2. **過去の対戦成績**\n3. **主要選手の状況**\n4. **戦術的な分析**\n5. **予想スコアと理由**\n\nより詳細な予測をご希望の場合は、Gemini APIキーを設定してください。`;
    }
    
    // 戦術分析
    if (lowerMessage.includes('戦術') || lowerMessage.includes('フォーメーション') || lowerMessage.includes('戦略')) {
        return `戦術分析を行います。\n\n${userMessage}について、以下の観点から分析いたします：\n\n1. **フォーメーションと戦術スタイル**\n2. **攻撃パターン**\n3. **守備システム**\n4. **主要選手の役割**\n5. **チームの強みと弱み**\n\nより詳細な戦術分析をご希望の場合は、Gemini APIキーを設定してください。`;
    }
    
    // 統計解説
    if (lowerMessage.includes('統計') || lowerMessage.includes('データ') || lowerMessage.includes('数字')) {
        return `統計データの分析を行います。\n\n${userMessage}について、以下の観点から分析いたします：\n\n1. **データの意味と重要性**\n2. **他選手・チームとの比較**\n3. **トレンドと傾向**\n4. **改善点や課題**\n5. **今後の展望**\n\nより詳細な統計分析をご希望の場合は、Gemini APIキーを設定してください。`;
    }
    
    // ファンタジーリーグ
    if (lowerMessage.includes('ファンタジー') || lowerMessage.includes('fantasy')) {
        return `ファンタジーリーグのアドバイスを提供します。\n\n${userMessage}について、以下の観点からアドバイスいたします：\n\n1. **選手選びの戦略**\n2. **ポジション別の重要度**\n3. **フォームと調子の見方**\n4. **試合日程の考慮**\n5. **キャプテン選びのコツ**\n\nより詳細なアドバイスをご希望の場合は、Gemini APIキーを設定してください。`;
    }
    
    // デフォルト応答
    return `サッカー分析AIアシスタントです！\n\n「${userMessage}」について分析いたします。\n\n現在、Gemini APIキーが設定されていないため、基本的な分析のみ提供しています。\n\nより詳細で専門的な分析をご希望の場合は、以下の手順でGemini APIキーを設定してください：\n\n1. Google AI StudioでAPIキーを取得\n2. .envファイルにGEMINI_API_KEYを追加\n3. サーバーを再起動\n\nそれまでは、基本的なサッカー分析をお楽しみください！⚽`;
}
