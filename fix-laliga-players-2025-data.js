...
/**
 * La Ligaの選手の2025/26シーズンデータをAPIから取得して修正するスクリプト
 */
...
async function updatePlayer2025Data(player) {
    ...
    try {
        ...
        // La Ligaのデータのみを抽出
        const laLigaStats = apiStats2025.filter(stat => 
            stat.league?.name === 'La Liga' || stat.league?.name === 'LaLiga'
        );
        
        if (laLigaStats.length === 0) {
            return { updated: false, reason: 'No La Liga data in API response' };
        }
        
        // 既存の2025/2026シーズンのLa Ligaデータを削除
        ...
        // APIから取得した2025年のLa Ligaデータを追加
        const changes = [];
        laLigaStats.forEach(apiStat => {
            const leagueName = apiStat.league?.name || 'La Liga';
        ...
        // stats配列も更新（La Ligaのみ）
        if (!player.stats || !Array.isArray(player.stats)) {
            player.stats = [];
        }
        
        // 既存の2025/2026シーズンのLa Liga statsを削除
        player.stats = player.stats.filter(s => 
            !((s.season === '2025/2026' || s.season === '2025/26') && 
              (s.leagueName === 'La Liga' || s.league === 'La Liga'))
        );
        
        // APIから取得した2025年のLa Ligaデータをstatsに追加
        laLigaStats.forEach(apiStat => {
            ...
            player.stats.push({
                season: '2025/2026',
                leagueName: apiStat.league?.name || 'La Liga',
                leagueId: apiStat.league?.id || null,
...
async function main() {
    ...
    console.log('🚀 La Ligaの選手の2025/26シーズンデータをAPIから取得して修正開始...\n');
    ...
    // La Ligaの2025/2026シーズンデータがある選手をフィルタリング
    ...
    console.log(`📊 La Ligaの2025/2026シーズンデータがある選手: ${playersWith2025LaLiga.length}名\n`);
    
    const playersToCheck = playersWith2025LaLiga.slice(startIndex, endIndex);
    ...
    for (let i = 0; i < playersToCheck.length; i++) {
        ...
        try {
            ...
            if (result.updated) {
                ...
                if (updatedCount <= 20) {
                    console.log(`✅ ${player.name}: ${result.changes.length}件のLa Ligaデータ更新`);
    ...
    if (endIndex < playersWith2025LaLiga.length) {
        console.log(`\n💡 続きを実行するには:`);
        console.log(`   node fix-laliga-players-2025-data.js ${endIndex} ${batchSize}\n`);
    }
}
